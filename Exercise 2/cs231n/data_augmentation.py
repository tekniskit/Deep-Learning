...
def random_crops(X, crop_shape):
  ...
  for n in xrange(N):
    dH = np.random.randint(0, H - HH)
    dW = np.random.randint(0, W - WW)
    for c in xrange(C):
      out[n, c, :, :] = X[n, c, dH:dH + HH, dW:dW + WW]
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  for n in xrange(N):
    rand_num = low + np.random.rand()*(high-low)
    out[n, :, :, :] = X[n, :, :, :] * rand_num
...
def random_tint(X, scale=(-10, 10)):
  ...
  for n in xrange(N):
    red = low + np.random.rand()*(high-low)
    green = low + np.random.rand()*(high-low)
    blue = low + np.random.rand()*(high-low)
    out[n, 0, :, :] = X[n, 0, :, :] + red
    out[n, 1, :, :] = X[n, 1, :, :] + green
    out[n, 2, :, :] = X[n, 2, :, :] + blue
